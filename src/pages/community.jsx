      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text)', fontSize: '22px', lineHeight: 1 }}
          >
            ←
          </button>
          <span style={{ fontSize: '24px' }}>{activeRoom.emoji}</span>
          <div>
            <p style={{ color: 'var(--text-h)', fontSize: '15px', fontWeight: '600', margin: 0 }}>
              {activeRoom.name}
            </p>
            <p style={{ color: 'var(--text)', fontSize: '11px', opacity: 0.5, margin: 0 }}>
              {activeRoom.members + 1} members
            </p>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', paddingBottom: '90px' }}>
          {roomMessages.length === 0 ? (
            <p style={{ color: 'var(--text)', opacity: 0.5, textAlign: 'center', marginTop: '40px' }}>
            ...
            roomMessages.map(msg => (
              <div key={msg.id} style={{ display: 'flex', gap: '10px', marginBottom: '22px' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ color: 'var(--text)', fontSize: '14px', lineHeight: '1.55', margin: 0, opacity: 0.85 }}>
                    {msg.text}
                  </p>
                </div>
              </div>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              padding: '10px 14px', color: 'var(--text-h)',
  ...
  return (
    <div style={{ padding: '28px 20px 16px' }}>
      ...
      <p style={{ color: 'var(--text)', fontSize: '13px', opacity: 0.6, marginBottom: '28px' }}>
        Find your people within the journey
      </p>
      ...
      {COMMUNITIES.map(c => {
        ...
        return (
          <div
            key={c.id}
            onClick={() => isJoined && setActiveRoom(c)}
            style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '3px' }}>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); if (!isJoined) setJoined(prev => ({ ...prev, [c.id]: true })) }}
                    style={{
                      fontSize: '12px', padding: '5px 14px', borderRadius: '8px', fontWeight: '600', flexShrink: 0,
                      border: isJoined ? '1px solid var(--border)' : '1px solid var(--accent)',
                      background: isJoined ? 'transparent' : 'var(--accent)',
                      color: isJoined ? 'var(--text)' : '#fff',
                ...
                <p style={{ color: 'var(--text)', fontSize: '11px', opacity: 0.45, marginBottom: '6px' }}>
                  {c.members + (isJoined ? 1 : 0)} members
                </p>
                <p style={{ color: 'var(--text)', fontSize: '13px', opacity: 0.72, lineHeight: '1.5', margin: 0 }}>
                  {c.description}
                </p>
                {isJoined && (
                  <p style={{ color: 'var(--accent)', fontSize: '12px', fontWeight: '500', marginTop: '8px', marginBottom: 0 }}>
                    Tap to enter room →
                  </p>
                )}
              </div>
